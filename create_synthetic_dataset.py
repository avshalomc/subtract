...
SMALL_BATCH = True # if true, creating dataset of size 8.
DATA_TYPE = '4'                    # '0'- democratic, '1'-republican, '2'-male, '3'-female , '4' - synthetic
...
PATHS_TO_DATA = {'0': './data/political_data/democratic_only.train.en',
                 ...
                 '4': './data/political_data/democratic_only.train.en'}
...
parser.add_argument('-synthetic_type', required=False, default='ident', help="overlap/twice/ident")
...
def main(args):
    ...
    # (1) Loading MT English Vocabulary
    ...
    # (2) preparing data for ROUGH-L test
    ...
    # (3) Creating a list of all sentences..
    all_sentences = []
    ...
    while True:
        ...
        # (4) Saves sline and tline which are sentences after removing the 'democratic ' starting
        clean_sline = sline[len(args.save_data)+1:] # cutting the start word from the sentence
        ...
        if 3 <= len1 <= args.seq_length and 3 <= len2 <= args.seq_length:
    ...
    while True:
        ...
        rouge_l_score = 0.66 # TODO: remove this change
        ...
        if count % 1000 == 0:
            ...
            for above_list, name in zip(lists, names):
                ...
                # TODO: increase overlapping
                print("Increasing overlaping")
                for k in range(len(sents1)):
                    if len(sents1[k]) < 3:
                        sents2[k] = sents1[k]
                    elif len(sents2[k]) < 3:
                        sents1[k] = sents2[k]
                    elif args.synthetic_type=='overlap':
                        overlap_len = min(len(sents1[k]), len(sents2[k]))
                        rand_len = random.randint(3, overlap_len)
                        rand_start_idx = random.randint(0,(overlap_len-rand_len))
                        sents1[k] = torch.cat((sents1[k][0:rand_start_idx], sents2[k][rand_start_idx:rand_start_idx+rand_len], sents1[k][rand_start_idx+rand_len:]), -1)
                    elif args.synthetic_type == 'twice':
                        sents1[k] = torch.cat((sents2[k], sents2[k]),0)[:args.seq_length]
                    elif args.synthetic_type == 'ident':
                        sents1[k] = sents2[k]
                ...
                if SMALL_BATCH:
                    train['sent1'] = train['sent1'][:1]
                    train['sent2'] = train['sent2'][:1]
                    valid['sent1'] = valid['sent1'][:1]
                    valid['sent2'] = valid['sent2'][:1]

                print('Saving data to \'' + args.save_data + '_%s_%s_train.pt\'...' % ('synthetic', args.synthetic_type))
                save_data = {'dicts': dicts,
                             'train': train,
                             'valid': valid,
                             }
                torch.save(save_data, '%s_%s1.train.pt' % ('synthetic', args.synthetic_type))

        # *******************************************************

        if len(above_025) > 100000:
            break
