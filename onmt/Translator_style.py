...
class Translator(object):
    def __init__(self, opt):
        ...
        encoder = style_decoder.onmt.Models.Encoder(enc_opt, self.src_dict)
        encoder.load_state_dict(encoder_check['encoder'])
        decoder = style_decoder.onmt.Models_decoder.Decoder(enc_opt, self.tgt_dict)
        decoder.load_state_dict(checkpoint['decoder'])
        model = style_decoder.onmt.Models.NMTModel(encoder, decoder)
    ...
    def buildData(self, srcBatch, goldBatch):
        srcData = [self.src_dict.convertToIdx(b,
                                              style_decoder.onmt.Constants.UNK_WORD) for b in srcBatch]
        tgtData = None
        if goldBatch:
            tgtData = [self.tgt_dict.convertToIdx(b,
                                                  style_decoder.onmt.Constants.UNK_WORD,
                                                  style_decoder.onmt.Constants.BOS_WORD,
                                                  style_decoder.onmt.Constants.EOS_WORD) for b in goldBatch]

        return onmt.Dataset(srcData, tgtData,
                            self.opt.batch_size, self.opt.cuda, volatile=True)

    def buildTargetTokens(self, pred, src, attn):
        tokens = self.tgt_dict.convertToLabels(pred, style_decoder.onmt.Constants.EOS)
        tokens = tokens[:-1]  # EOS
        if self.opt.replace_unk:
            for i in range(len(tokens)):
                if tokens[i] == style_decoder.onmt.Constants.UNK_WORD:
    ...
    def translateBatch(self, srcBatch, tgtBatch):
        ...
        padMask = srcBatch.data.eq(style_decoder.onmt.Constants.PAD).t()
        ...
        if tgtBatch is not None:
            ...
            for dec_t, tgt_t in zip(decOut, tgtBatch[1:].data):
                ...
                scores.masked_fill_(tgt_t.eq(style_decoder.onmt.Constants.PAD), 0)
                goldScores += scores
        ...
        padMask = srcBatch.data.eq(style_decoder.onmt.Constants.PAD).t().unsqueeze(0).repeat(beamSize, 1, 1)
        ...
        for b in range(batchSize):
            ...
            valid_attn = srcBatch.data[:, b].ne(style_decoder.onmt.Constants.PAD).nonzero().squeeze(1)
